        ...
        private static void ParseFieldFromXml(XElement xRule, RuleComposite result, List<DocumentSchema> fields)
        {
            ...
            try
            {
                ...
                if (field == null || string.IsNullOrEmpty(field.Value))
                    throw new RuleParseException(lineNumber, "Не указано поле, над которым необходимо выполнять текущие проверки.");
                ...
                if (required != null && required.Value.Equals("true") && isEmpty != null && isEmpty.Value.Equals("true"))
                    throw new RuleParseException(lineNumber, "Одновременно задана проверка на обязательность и на пустоту поля");
                ...
                foreach (var compare in compares)
                {
                    ...
                    if (compareField != null)
                    {
                        if (compareOperator == null)
                            throw new RuleParseException(lineNumber, "Не указан оператор сравнения.");

                        if (compareType == null)
                            throw new RuleParseException(lineNumber, "Не указан тип сравнения.");

                        if (!_operatorsMap.ContainsKey(compareOperator.Value))
                            throw new RuleParseException(lineNumber, "Указан неизвестный оператор сравнения.");

                        if (!_typesMap.ContainsKey(compareType.Value))
                            throw new RuleParseException(lineNumber, "Указан неизвестный тип данных для сравнения.");
                ...
                foreach (var range in ranges)
                {
                    ...
                    if (rType == null)
                        throw new RuleParseException(lineNumber, "Не указан тип сравнения для проверки диапазона значений.");

                    if (!_typesMap.ContainsKey(rType.Value))
                        throw new RuleParseException(lineNumber, "Указан неизвестный тип сравнения для проверки диапазона значений.");
                ...
                foreach (var fieldType in fieldTypes)
                {
                    ...
                    {
                        throw new RuleParseException(lineNumber, string.Format("Неизвестный тип {0}", requiredType.Value));
                    }
            ...
            {
                throw new RuleParseException(lineNumber, "Ошибка разбора правила-валидации из xml.", ex);
            }
        }
