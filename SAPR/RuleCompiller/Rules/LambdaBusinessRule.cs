			...
			: base(codeTreeHandler, xFieldDefinition, persistValue)
		{
			...
			try
			{
				...
				if (expression == null || string.IsNullOrEmpty(expression.Value))
					throw new Helpers.RuleParseException(lineNumber, "Для бизнес-правила должно быть указано выражение для вычисления значения поля.");

				ExpressionString = expression.Value;
			}
			catch (Exception ex)
			{
				throw new Helpers.RuleParseException(lineNumber, "Ошибка разбора бизнес-правила на основе лямбда выражений из xml.", ex);
			}
		}
