	...
	public abstract class BusinessRule : Rule
	{
		...
		public static Rule Parse(XElement xRule, CodeTreeHandler codeTreeHandler)
		{
			...
			try
			{
				...
				if (mode == null)
					throw new Exception(lineNumber + "Не указан тип бизнес-правила (временное значение или сохраняемое).");
				...
				foreach (var target in xRule.XPathSelectElements("./target"))
				{
					...
					if (function == null && expression == null)
						throw new Exception(lineNumber + "Не указано объявление поля с параметрами вычисления значения и его хранения.");

					if (function != null && expression != null)
						throw new Exception(lineNumber + "Указаны неверные параметры бизнес-правила: поле не может одновременно быть вычислимым через SQL-функции и через Lambda-выражения.");
			...
			{
				throw new Exception(lineNumber + "Ошибка разбора бизнес-правила из xml.", ex);
			}
		}
